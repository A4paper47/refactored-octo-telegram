...
## V9 Update — Trait/Rarity Staff + Burnout + Reputation + Client Layer

Layer gameplay sekarang dah lebih matang dan lebih “game-like”, sambil masih kekal rapat dengan domain asal VO workflow.

### Apa yang ditambah

- setiap `Staff` kini ada:
  - `rarity`
  - `traits`
  - `burnout`
- setiap `Mission` kini ada:
  - `client_name`
  - `client_tier`
  - `reputation_reward`
- `GameState` kini simpan:
  - `reputation`
  - `clients_seen`
- mission success / failure ubah reputation studio
- market quality dipengaruhi studio tier + reputation
- burnout memberi penalti pada prestasi staff
- lounge / next day membantu recovery burnout
- command baru:
  - `/clients`
  - `/reputation`
...
Sebelum ini loop game lebih fokus pada:
- pilih mission
- assign cast
- submit
- dapat reward

Sekarang loop dah ada strategi jangka sederhana:
- jaga staff jangan burnout
- pilih antara staff rarity tinggi vs kos salary lebih besar
- bina reputation untuk unlock client tier lebih baik
- fikir sama ada nak push urgent mission atau stabilkan studio dulu
